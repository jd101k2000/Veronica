...
function weaverii_process_options_themes() {
	...
	if (current_user_can( 'manage_options' ))
	    update_option(apply_filters('weaver_options','weaverii_settings_backup'),$weaverii_opts_cache);
	weaveriip_save_opts_backup();
    }

    if (weaverii_submitted('restore_mytheme')) {	// invoked from Save/Restore tab
	global $weaverii_opts_cache;
	$saved = get_option( apply_filters('weaver_options','weaverii_settings_backup') ,array());
	if (!empty($saved)) {
...
function weaverii_activate_subtheme($theme) {
    ...
    */
    ...
    weaverii_setopt('wii_last_option','Weaver II');	// assume always ok if from file
