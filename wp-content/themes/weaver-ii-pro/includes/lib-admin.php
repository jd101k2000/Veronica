...
function weaverii_sapi_form_bottom($form_name='end of form') {
	...
	'wii_theme_description', 'wii_hide_old_weaver', 'wii_theme_filename', '_wii_save_mods_basic',
...
<?php
    ...
    weaverii_setopt('wii_last_option','Weaver II');	// Safety check for limited PHP $_POST variables
    echo ("</form> <!-- $form_name -->\n");
}
...
function weaverii_check_version($force=false) {
	...
	$msg = ') is available in the member downloads area at <a href="http://pro.weavertheme.com" target="_blank">pro.WeaverTheme.com</a>. If you have installed the "Weaver II Theme Extras" plugin,
you can download the new version, and easily update using the option near the bottom of the Save/Restore tab.';
    } else {
    ...
    $existing_vers = weaverii_getopt('wii_version');
	...
	if ($version != $existing_vers) {
	    weaverii_setopt('wii_version',$version);	// update to latest...
	    weaverii_save_opts('weaverii_check_version');
	    weaverii_save_msg('The options settings have been initialized or updated for the current version of ' . WEAVERII_THEMENAME . '. (' . $version . ')');
	}
    }
    return '';
}
...
?>
    ...
    $val = weaverii_getopt('wii_hide_old_weaver');
...
?>
	...
	click the ? help button for instructions for upgrading. This message
...
<?php
	...
	$css_rows = 1;
    if ($css_id_text && !weaverii_getopt( '_wii_hide_auto_css_rules' ) && !weaverii_hide_advanced()) { ?>
	...
	<td>
	    ...
	    <textarea name="<?php weaverii_sapi_main_name($css_id); ?>" rows=<?php echo $css_rows;?> style="width: 85%"><?php weaverii_esc_textarea($css_id_text); ?></textarea>
	</td>
    </tr>
<?php
    } else {
...
?>
	...
	<strong>Aspen Theme Installed</strong> - You can convert most of your Weaver II settings to Aspen Theme settings by using these options. See Aspen Theme help for more info.
	<br />
	<a href="<?php echo $download_path . '?_wpnoncea=' . $nonce; ?>"><img src="<?php echo $download_img_path; ?>" alt="download" />&nbsp; <strong>Download and Convert</strong>&nbsp;</a> - <strong><em>All</em></strong> current settings to file <strong>aspen-converted-settings.abu</strong> on your computer.
<br />
<a href="<?php echo $download_path . '?_wpnonceat=' . $nonce;?>"><img src="<?php echo $download_img_path; ?>" alt="download" />&nbsp;<strong>Download and Convert</strong></a>&nbsp; - <strong><em>Only theme related</em></strong> current settings to file <strong>aspen-converted-settings.ath</strong> on your computer.
    </div>
    <br />
<?php
    }
...
function weaverii_set_current_to_serialized_values($contents)  {
    ...
    weaverii_setopt('wii_last_option','Weaver II');
