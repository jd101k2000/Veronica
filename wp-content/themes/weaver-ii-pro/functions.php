...
function weaverii_post_title($before='', $after='', $single = '') {
    ...
    if (weaverii_is_checked_post_opt('hide_post_title') || weaverii_sc_getopt('hide_title')) return;
...
<?php
	...
	&& weaverii_sc_getopt('show_avatar') != 'no') {
?>
...
function weaverii_posted_on($type='') {
    ...
    printf( __( '<span class="sep">Posted on </span><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a><span class="by-author"> <span class="sep"> by </span> <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>','weaver-ii'),
...
if (!function_exists('weaverii_show_only_title')) {
    ...
    if (weaverii_sc_getopt('show') == 'title') return true;
    if (weaverii_sc_getopt('showposts')) return false;
    if (weaverii_get_per_page_value('wvr_pwp_type') == 'title_featured') {
        // title has been displayed - add the featured image after
        weaverii_use_fi_in_content();
        return true;
    }
    return weaverii_get_per_page_value('wvr_pwp_type') == 'title';
}
}

if (!function_exists('weaverii_use_fi_in_content')) {
function weaverii_use_fi_in_content($page_type = 'post') {
    global $weaverii_header;
    $default = 'thumbnail';
    $width = '';
...
function weaverii_quickcache_md5_salt($ignore) {
    ...
    if (weaverii_in_mobile_view()) $full = '';
    else $full = 'full';
...
function weaverii_add_admin() {	// action definition
    ...
    // 'edit_theme_options' works for both single and multisite
    	$page = add_theme_page('WeaverII', apply_filters('weaver_admin_title',WEAVERII_THEMENAME . ' ' . weaverii_t_('Admin' /*a*/ )), 'edit_theme_options', 'WeaverII', 'weaverii_admin');
    ...
    $page2 = add_theme_page('WeaverII_Shortcodes', '&nbsp;&rArr; ' . weaverii_t_('Shortcodes + Pro' /*a*/ ), 'edit_theme_options', 'WeaverII_Shortcodes', 'weaverii_admin_sc');
    /* using registered $page handle to hook stylesheet loading for this admin page */
...
function weaverii_enqueue_scripts() {	// action definition
    ...
    wp_enqueue_script('weaverJSLib', get_template_directory_uri().'/js/weaverjslib'.WEAVER_MINIFY.'.js',array('jquery'),WEAVERII_VERSION);
    ...
    if (is_page_template('paget-html.php'))
        wp_enqueue_script('weaverPrettify', get_template_directory_uri().'/js/theme/prettify'.WEAVER_MINIFY.'.js',array(),WEAVERII_VERSION); // only for html page
...
function weaverii_facebook_meta() {
    ...
    */
    ...
    if ($siteimg != '' ) {
        if (!weaverii_getopt_checked('_wii_hide_metainfo') && !function_exists('wpseo_get_value')) {
?>
...
require_once(dirname( __FILE__ ) . '/includes/weaverii-sc-basic.php'); // pro runtime library
...
if (is_admin() && isset($_GET['activated'] ) && $GLOBALS['pagenow'] == "themes.php" ) {
	//Do redirect
    wp_redirect( admin_url( 'themes.php?page=WeaverII' ) );
    exit;
}
